...
describe("Tests getters and setters, and methods", function() {
  ...
  it("attributeChangedCallback", function(done) {
    comp.attributeChangedCallback("disabled", "", "");
    assert.equal(comp.button.disabled, true);
    done();
  });
  it("attributeChangedCallback", function(done) {
    comp.attributeChangedCallback("disabled", "", "test");
    assert.equal(comp.button.disabled, false);
    done();
  });
  it("attributeChangedCallback", function(done) {
    comp.attributeChangedCallback("autofocus", "", "");
    assert.equal(comp.button.autofocus, true);
    done();
  });
  it("attributeChangedCallback", function(done) {
    comp.attributeChangedCallback("autofocus", "", "test");
    assert.equal(comp.button.autofocus, false);
    done();
  });
  it("attributeChangedCallback", function(done) {
    comp.attributeChangedCallback("type", "", "button");
    assert.equal(comp.button.type, "button");
    done();
  });
  it("attributeChangedCallback", function(done) {
    comp.attributeChangedCallback("bootstrap", "", "btn");
    assert.equal(comp.button.className, "btn");
    done();
  });
  ...
  it("test round getter & setter", function(done) {
    comp.round = true;
    assert.equal(comp.round , true);
    comp.round = false;
    assert.equal(comp.round , false);
  ...
  it("test circle getter & setter", function(done) {
    comp.circle = true;
    assert.equal(comp.circle , true);
    comp.circle = false;
    assert.equal(comp.circle , false);
  ...
  it("test size getter & setter", function(done) {
    comp.size = "true";
    assert.equal(comp.size , "true");
    comp.size = "";
    assert.equal(comp.size , null);
    done();
  });
  ...
  it("test _onClick()", function(done) {
    comp.type = "reset";
    comp._parentForm = false;
    assert.notEqual(comp._onClick("", ""), true);
    comp.type = "submit";
    assert.notEqual(comp._onClick("", ""), true);
    done();
  });
