...
`;export class MeatBoilerplate extends HTMLElement{/**
   ...
   */constructor(){cov_1tgj84hqau.f[0]++;cov_1tgj84hqau.s[2]++;super();cov_1tgj84hqau.s[3]++;this.shadow=this.attachShadow({mode:"open"});cov_1tgj84hqau.s[4]++;this.shadow.appendChild(template.content.cloneNode(true));}/**
   * Live-cycle method called when the custom element is loaded, often used for initialization
   */connectedCallback(){cov_1tgj84hqau.f[1]++;}/**
   * Tell the webcomponent to observe these attributes, if any of them are added, changed, or removed, then
   * call attributeChangedCallback(name, oldVal, newVal)
   */static get observedAttributes(){cov_1tgj84hqau.f[2]++;cov_1tgj84hqau.s[5]++;return[];}/**
   * Called whenever one of the attributes specified in observedAttributes() is changed
   * @param {string} name
   * @param {string} oldVal
   * @param {string} newVal
   */attributeChangedCallback(name,oldVal,newVal){cov_1tgj84hqau.f[3]++;}}cov_1tgj84hqau.s[6]++;window.customElements.define("meat-boilerplate",MeatBoilerplate);