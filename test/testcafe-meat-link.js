...
fixture`Running tests for meat-link` // declare the fixture
  .page`http://127.0.0.1:8080/web_components/meat-link/meat-link-test.html`;
...
test("Correct CSS color", async t => {
  ...
  await t.expect(component.getStyleProperty("color")).eql("rgb(0, 0, 0)");
});

test("Cannot click disabled link", async t => {
  const component = await Selector(() =>
    document.querySelector("#disabled-test").shadowRoot.querySelector("a")
  );
  await t
    .hover(component)
    .expect(component.getStyleProperty("cursor"))
    .eql("not-allowed");
});

test("Test underline only on hover", async t => {
  const component = await Selector(() =>
    document.querySelector("#underline-hover").shadowRoot.querySelector("a")
  );

  await t
    .expect(component.getStyleProperty("text-decoration"))
    .contains("none");
...
test("Test never hover attribute", async t => {
  const component = await Selector(() =>
    document.querySelector("#underline-never").shadowRoot.querySelector("a")
  );
...
test("Test always underline attribute", async t => {
  const component = await Selector(() =>
    document.querySelector("#underline-always").shadowRoot.querySelector("a")
  );
  await t
    .expect(component.getStyleProperty("text-decoration"))
    .contains("underline");
...
test("test-white has attributes underline and color", async t => {
  ...
  await t
    .expect(component.getAttribute("underline"))
    .eql("always")
    .expect(component.getAttribute("color"))
    .eql("white");
});
...
test("test-white is white CSS", async t => {
  const component = await Selector(() =>
    document.querySelector("#test-white").shadow.querySelector("a")
  );
  await t.expect(component.getStyleProperty("color")).eql("rgb(255, 255, 255)");
});
...
test("test-red has attributes underline and color", async t => {
  const component = getElementById("#test-red");
  await t.expect(component.getAttribute("color")).eql("red");
});

test("Can click link", async t => {
  const component = await Selector(() =>
    document.querySelector("#color").shadow.querySelector("a")
  );
  await t.click(component);

  const newPage = await Selector(() => document.querySelector("#text-test"));
  await t.expect(newPage.textContent).eql("Basic buttons");
});
